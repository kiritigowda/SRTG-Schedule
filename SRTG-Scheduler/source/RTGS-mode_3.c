	...
	scheduledResourceNode **jobScheduledQueueList)
{
	...
	int scheduleMethod = RTGS_SCHEDULE_METHOD_ALAP;
		...
		(processorsReleaseTime + jobAttributesList[jobNumber].execution_time) <= jobAttributesList[jobNumber].deadline)
	{
		...
		if (processorsAvailable >= jobAttributesList[jobNumber].processor_req)
		{
			...
			job_queue_handler(processorsInUse, jobReleaseTime, processorReleaseTime,
							  scheduleMethod, jobNumber, jobScheduledQueueList);
			return processorsAvailable;
		}
		// condition 4
...
static int Mode_3_ALAP(
	...
	scheduledResourceNode **jobScheduledQueueList)
{
	int localProcessors = 0, jobReleaseTime = 0, processorReleaseTime = 0, processorsAllocated = 0;
		...
		while (localProcessorsAllocatedList != NULL)
		{
			...
			else
			{
				...
				if (localProcessors >= jobAttributesList[jobNumber].processor_req)
				{
					...
					int scheduleMethod = RTGS_SCHEDULE_METHOD_AEAP;
					...
					queue_job_execution(processorsInUse, processorReleaseTime, presentTimeL, scheduleMethod, jobNumber, processorsAllocatedList);
					job_queue_handler(processorsInUse, jobReleaseTime, presentTimeL, scheduleMethod, jobNumber, jobScheduledQueueList);
					break;
				}
				else
				{
			...
			else
			{
				...
				if (localProcessors >= jobAttributesList[jobNumber].processor_req)
				{
					...
					if (jobAttributesList[jobNumber].processor_req <= availAlapProcessors &&
						(futureRelease == 0 || (jobReleaseTime + jobAttributesList[jobNumber].execution_time) <= futureRelease))
					{
						localProcessorsAllocatedList->processors_allocated = localProcessors - jobAttributesList[jobNumber].processor_req;
						processorsDistList = clean_list(processorsDistList);
						...
						queue_job_execution(processorsInUse, processorReleaseTime, presentTimeL,
											scheduleMethod, jobNumber, processorsAllocatedList);
						job_queue_handler(processorsInUse, jobReleaseTime, presentTimeL,
										  scheduleMethod, jobNumber, jobScheduledQueueList);
						break;
					}
					// condition 2
					else if (processorReleaseTime <= GLOBAL_preScheduleList->data)
					{
						localProcessorsAllocatedList->processors_allocated = localProcessors - jobAttributesList[jobNumber].processor_req;
						processorsDistList = clean_list(processorsDistList);
	...
	scheduledResourceNode **jobScheduledQueueList)
{
	...
	{
		processorsAvailable = Mode_3_AEAP(jobAttributesList, jobNumber, presentTime, processorsAvailable, processorsAllocatedList, jobScheduledQueueList);
	}
	else if (jobAttributesList[jobNumber].processor_req >= GLOBAL_DELAY_SCHEDULE_PROCESSOR_LIMIT && GLOBAL_preScheduleList == NULL)
	{
		processorsAvailable = Mode_3_ALAP(jobAttributesList, jobNumber, presentTime, processorsAvailable, processorsAllocatedList, jobScheduledQueueList);
	}
	else
	{
		...
		processorsAvailable = Mode_3_ALAP_advanced(jobAttributesList, jobNumber, presentTime,
	...
	scheduledResourceNode **jobScheduledQueueList)
{
		...
		if (GLOBAL_preScheduleList == NULL)
		{
			...
			if (jobAttributesList[jobNumber].processor_req < GLOBAL_DELAY_SCHEDULE_PROCESSOR_LIMIT)
			{
				if (jobAttributesList[jobNumber].execution_time + presentTimeL <= jobAttributesList[jobNumber].deadline)
				{
					...
					processorsInUse = jobAttributesList[jobNumber].processor_req;
					processorReleaseTime = jobAttributesList[jobNumber].execution_time + presentTimeL;
					scheduleMethod = RTGS_SCHEDULE_METHOD_IMMEDIATE;
					// Job call for the GPU to handle the given Jobs and number of blocks//
					queue_job_execution(processorsInUse, processorReleaseTime, presentTimeL, scheduleMethod, jobNumber, processorsAllocatedList);
					jobAttributesList[jobNumber].schedule_hardware = RTGS_HARDWARE_GPU;
					jobAttributesList[jobNumber].rescheduled_execution = -1;
					jobAttributesList[jobNumber].scheduled_execution = presentTimeL;
					jobAttributesList[jobNumber].completion_time = jobAttributesList[jobNumber].execution_time + presentTimeL;
					GLOBAL_GPU_JOBS++;
					if (GLOBAL_RTGS_DEBUG_MSG > 1)
					{
			...
			else
			{
				...
				processorsAvailable = Mode_3_ALAP(jobAttributesList, jobNumber, presentTime,
												  processorsAvailable, processorsAllocatedList, jobScheduledQueueList);
			}
		}
		else
	...
	else
	{
		...
		processorsAvailable = Mode_3_Processors_Unavailable(jobAttributesList, jobNumber, presentTime,
															processorsAvailable, processorsAllocatedList, jobScheduledQueueList);
	}
	return processorsAvailable;
}
...
/***********************************************************************************************************
MODE 3 Enhanced Method FUNCTIONS
**********************************************************************************************************/
static int mode_3_enhanced_method(
	jobAttributes *jobAttributesList,
	int jobNumber,
	int processorsAvailable,
	int presentTime,
	...
	scheduledResourceNode **jobScheduledQueueList)
{
	...
	if (GLOBAL_RTGS_METHOD >= RTGS_METHOD_ENHANCED && jobAttributesList[jobNumber].processor_req_m > 0)
	{
		...
		processorsAvailable = Mode_3_book_keeper(jobAttributesList, jobNumber, processorsAvailable, presentTime,
...
int RTGS_mode_3(char *jobsListFileName, char *releaseTimeFilename)
{
	...
	for (int presentTime = 0; presentTime < MAX_RUN_TIME; presentTime++)
	{
		...
		{
			printf("Retrieve_processors ERROR - PresentTime:%d GCUs Available:%d\n", presentTime, processorsAvailable);
		...
		if (processorsAvailable < 0)
		{
			printf("Dispatch_queued_kernels ERROR - PresentTime:%d GCUs Available:%d\n", presentTime, processorsAvailable);
			return RTGS_ERROR_NOT_IMPLEMENTED;
		}
		...
		if (releaseTimeInfo[numReleases].release_time == presentTime)
		{
			...
			if (releaseTimeInfo[numReleases].num_job_released == 1)
			{
				...
				if (GLOBAL_RTGS_METHOD > RTGS_METHOD_BASE && jobAttributesList[jobNumber].schedule_hardware == RTGS_HARDWARE_CPU)
				{
					...
					processorsAvailable = mode_3_enhanced_method(jobAttributesList, jobNumber, processorsAvailable, presentTime,
																 &processorsAllocatedList, &jobScheduledQueueList);
			...
			else if (releaseTimeInfo[numReleases].num_job_released == 2)
			{
				...
				if (jobAttributesList[k1].deadline <= jobAttributesList[k2].deadline)
				{
					// handling the released jobAttributesList by the book-keeper
					int64_t start_t = RTGS_GetClockCounter();
					processorsAvailable = Mode_3_book_keeper(jobAttributesList, k1, processorsAvailable, presentTime, &processorsAllocatedList, &jobScheduledQueueList);
					int64_t end_t = RTGS_GetClockCounter();
					int64_t freq = RTGS_GetClockFrequency();
					float factor = 1000.0f / (float)freq; // to convert clock counter to ms
					float SchedulerOverhead = (float)((end_t - start_t) * factor);
					...
					{
						int64_t start_t_e = RTGS_GetClockCounter();
						processorsAvailable = mode_3_enhanced_method(jobAttributesList, k1, processorsAvailable, presentTime,
																	 &processorsAllocatedList, &jobScheduledQueueList);
						int64_t end_t_e = RTGS_GetClockCounter();
						float SchedulerOverhead_e = (float)((end_t_e - start_t_e) * factor);
					...
					processorsAvailable = Mode_3_book_keeper(jobAttributesList, k2, processorsAvailable, presentTime, &processorsAllocatedList, &jobScheduledQueueList);
					...
					{
						int64_t start_t_e = RTGS_GetClockCounter();
						processorsAvailable = mode_3_enhanced_method(jobAttributesList, k2, processorsAvailable, presentTime,
																	 &processorsAllocatedList, &jobScheduledQueueList);
						int64_t end_t_e = RTGS_GetClockCounter();
						float SchedulerOverhead_e = (float)((end_t_e - start_t_e) * factor);
			...
			{
				printf("RTGS Mode 3 ERROR --  RTGS_ERROR_NOT_IMPLEMENTED\n");
				return RTGS_ERROR_NOT_IMPLEMENTED;
			}
