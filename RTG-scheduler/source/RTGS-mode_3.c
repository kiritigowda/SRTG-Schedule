...
int RTGS_mode_3(char *kernel_file, char *Releasetime_file)
{
	...
	for (int present_time = 0; present_time < rt; present_time++)
	{
		...
		if (GLOBAL_RELEASE_TIME[present_time] == 1)
		{
			...
			processors_available = Mode_3_book_keeper(kernel_info_list, kernel_number, processors_available, present_time, &processor_alloc_list, &kernel_queue_list); // handling the released kernel_info_list by the book-keeper
			kernel_number++;
		}
		...
		else if (GLOBAL_RELEASE_TIME[present_time] == 2)
		{
			...
			{
				processors_available = Mode_3_book_keeper(kernel_info_list, k1, processors_available, present_time, &processor_alloc_list, &kernel_queue_list); // handling the released kernel_info_list by the book-keeper
				processors_available = Mode_3_book_keeper(kernel_info_list, k2, processors_available, present_time, &processor_alloc_list, &kernel_queue_list); // handling the released kernel_info_list by the book-keeper
			}
...
int Mode_3_book_keeper(kernelInfo* kernel_info_list, int kernel_number, int processors_available, int present_time, scheduledNode **processor_alloc_list, scheduledNode **kernel_queue_list) {
	...
	if (kernel_info_list[kernel_number].processor_req <= processors_available) // If processors available is greater than the required processors by the kernel_info_list
	{
		...
		if (GLOBAL_ALAP_LIST == NULL) {
...
#if DEBUG_MESSAGES
				printf("\n||---ALAP is set!! -->The Kernel:%d SATISFIED CONDITION 1 or 2", kernel_number);
#endif
			...
			{
#if DEBUG_MESSAGES
				printf("\n\n@@ Kernel-%d REJECTED as ALAP kernel_info_list is set@@\n\n", kernel_number);
#endif
				GLOBAL_CPU_KERNELS++;
			}
	...
	else if (kernel_info_list[kernel_number].processor_req > processors_available) // If processors available is greater than the required processors by the kernel_info_list
	{
		processors_available = Mode_3_Processors_Unavailable(kernel_info_list, kernel_number, present_time, processors_available, processor_alloc_list, kernel_queue_list); // Schedule the kernel_info_list to be released in a future time
	}
...
#if DEBUG_MESSAGES
				printf("\n\n||---Mode_3_AEAP-->The Kernel:%d Cannot be scheduled Mode_3_AEAP*****|", kernel_number);
#endif
				GLOBAL_CPU_KERNELS++;
...
#if DEBUG_MESSAGES
					printf("\n||---Mode_3_AEAP-->The Kernel:%d scheduled Mode_3_AEAP -->---||", kernel_number);
#endif
			...
			if ((temp->processor_release_time + kernel_info_list[kernel_number].execution_time) > kernel_info_list[kernel_number].deadline) {
				...
				if (Pro >= kernel_info_list[kernel_number].processor_req) {
					...
					if ((kernel_info_list[kernel_number].processor_req + given) <= Pl) {
...
#if DEBUG_MESSAGES
						printf("\n||---Mode_3_AEAP with ALAP Condition-1 -->The Kernel:%d scheduled Mode_3_AEAP -->---||", kernel_number);
#endif
						Queue_kernel_execution(processorReleased, processor_release_time, presentTime, schedule_method, kernel_number, processor_alloc_list);
						Kernel_queue_handler(processorReleased, kernel_release_time, presentTime, schedule_method, kernel_number, kernel_queue_list);
						return processors_available;
						...
						P_Given_list = clean_list(P_Given_list);
#if DEBUG_MESSAGES
						printf("\n||---Mode_3_AEAP with ALAP Condition-2 -->The Kernel:%d scheduled Mode_3_AEAP -->---||", kernel_number);
#endif
...
#if DEBUG_MESSAGES
						printf("\n!!!---Mode_3_AEAP with ALAP is not Possible for the Kernel:%d-->---!!!", kernel_number);
